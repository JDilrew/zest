...
export class ZestMocker {
  constructor(envGlobal = globalThis) {
    this._env = envGlobal;
    this._state = new WeakMap(); // Mock -> { calls, results, lastCall }
    this._config = new WeakMap(); // Mock -> { impl }
  ...
  /** Minimal spyOn: replaces a method with a mock that calls through by default. */
  spyOn(obj, key) {
  ...
  /** Type guard like jest.isMockFunction */
  isMockFunction(fn) {
...
// handy singleton + named exports like jest-mock
